...
const SetState = () => {
    ...
    // add student list in table 
    const addStudentList = () => {
        setStudetnList(prevObject => ([...prevObject, studentInfo]))
        setStudentInfo({ StudeName: "", Mark: "", Grade: "" })
    }
        ...
        <div>
            <div className="container">
                ...
                <div className="row">
                    ...
                    <div className="col-3">
                        <button type="button" className='btn btn-primary' onClick={addStudentList}> Save</button>
                    </div>
                </div>
                <div className="row">
                    <div className="col-12 mt-3">
                        <table className='table table-bordered table-hover text-center'>
                            ...
                            <tbody>
                                {studetnList.map((item, index) => (
                                    <tr key={index}>
                                        <td>{index + 1}</td>
                                        <td>{item.StudeName}</td>
                                        <td>{item.Mark}</td>
                                        <td>{item.Grade}</td>
                                    </tr>
                                ))}
